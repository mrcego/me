      ...
      <div class="container mx-auto max-w-4xl space-y-10 md:space-y-14">
        <div class="space-y-6 md:space-y-8">
          ...
          <div class="flex flex-wrap gap-3 text-sm font-bold text-muted">
            <AppBadge
              :label="t(copyKey('role'))"
              variant="outline"
              size="md"
              class="font-mono text-xs"
            />
            <AppBadge
              :label="t(copyKey('period'))"
              variant="outline"
              size="md"
              class="font-mono text-xs"
            />
        ...
        <section class="space-y-6 md:space-y-8">
          <AppSurface
            ...
            variant="glass"
            rounded="3xl"
            class="surface-card p-6 md:p-8 space-y-3 border-foreground/5"
          >
        ...
        <section v-if="highlightItems.length">
          <AppSurface
            as="div"
            variant="glass"
            rounded="3xl"
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <AppSurface
              ...
              rounded="3xl"
              interactive
              class="surface-card group relative p-6 sm:p-7 md:p-8 flex flex-col justify-between border-foreground/5 min-w-0"
            >
              <div class="space-y-4">
                ...
                <ul class="flex flex-wrap gap-1.5 pt-1" :aria-label="t('caseStudies.tagsLabel')">
                  <li v-for="tag in otherTags(other.slug)" :key="tag">
                    <AppBadge
                      :label="tag"
                      variant="subtle"
                      size="sm"
